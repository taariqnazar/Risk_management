...
portfolio_value <- function(samples, mean, sigma, p, c, leverage = 2, r_bar = 0.005) {
  ...
  for (sample_col in 1:dim(samples)[2]) { 
    ...
    # Consider the base case when V_0 = G_0 = 0, then Vk+m ≥ Gk+m
    V <- leverage * (0 + 1000 - G_func(1, r_bar)*exp(-r)) * R_stock + 
      (0 + 1000 - leverage*(0 + 1000 - G_func(1, r_bar)*exp(-r))) * R_ZCP
    for (year in 2:30) {
      print(paste(toString(V), "V"))
      print(paste(toString(G_func(year-1, r_bar)), "G"))
      if (V < G_func(year-1, r_bar)) {
        print("test")
        V = V + 1000 * R_ZCP
      }
      else {
        R_stock <- exp(mean + sigma*samples[year,sample_col])
        risky_asset <- leverage * (V + 1000 - G_func(year, r_bar)*exp(-r)) * R_stock
        bank_account <- (V + 1000 - 2*(V + 1000 - G_func(year, r_bar)*exp(-r))) * R_ZCP
